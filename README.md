...
For deeper language-aware retrieval, `data/pine_v6_language_reference_taxonomy.json` expands the taxonomy with defined namespaces, invariants, built-in variable families, syntax constructs, type categories, operator categories, and routing facets. `data/pine_v6_self_auditing_engineering_standard.json` is the canonical self-auditing standard that binds requirements to owners, validation, evidence, falsification criteria, confidence, temporal dependency management, and closure mechanics.

## Audit

Run the integrated white-glove audit before treating the builder or taxonomy files as closed:

```bash
python scripts/audit_pine_v6_assets.py
```

The audit validates code syntax, JSON structure, taxonomy coverage thresholds, README traceability, governance fields, placeholder absence, and that incomplete generated `output/` artifacts are not committed as canonical deliverables.
